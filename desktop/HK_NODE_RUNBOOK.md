...
### ⚠️ 下单页最容易选错的 4 项（2026-09-11 实测踩点）
...
| **套餐** | 2 vCPU / **1 GB** / 30 GB（¥34 档） | 2 vCPU / **2 GB** / 40 GB | 1G 跑 Docker 构建（含 bgutil PO token 编译）极易 OOM，运行期 yt-dlp+ffmpeg 更悬；脚本里专门建 2G swap 就是为 2G 内存机准备的 |
| **镜像** | Alibaba Cloud Linux 3.x | **Ubuntu 22.04（系统镜像）** | `deploy_hk.sh` 第 7 步装 Caddy/自动 HTTPS 走的是 `apt-get`；Alibaba Cloud Linux 是 `dnf`，该步直接失败（主部署步骤虽不挑系统，但 HTTPS 会缺） |
