...
@router.post("/api/cloud/baidu/share/list")
def cloud_baidu_share_list(payload: app.BaiduShareListRequest):
    """列出分享链接里的文件（看分享内容本身无需登录）。"""
    if not app.BAIDU_ENABLED:
        raise app.HTTPException(status_code=503, detail="该实例未配置百度网盘应用凭据")
    if not payload.url.strip():
        raise app.HTTPException(status_code=400, detail="缺少分享链接")
    try:
        result = app._baidu_provider.share_list(payload.url, payload.pwd, payload.dir)
    except app.CloudError as exc:
        raise app.HTTPException(status_code=502, detail=exc.message + (("：" + exc.hint) if exc.hint else ""))
    return result

@router.post("/api/cloud/baidu/share/download")
def cloud_baidu_share_download(payload: app.BaiduShareDownloadRequest):
    ...
    def _worker() -> None:
        dest = app.DOWNLOAD_DIR / "baidu" / "share" / name
        try:
            with app._baidu_dl_lock:
                app._baidu_dl_tasks[tid].update(status="transferring")
            ...
            app._baidu_provider.download_share(
    ...
    app.threading.Thread(target=_worker, name=f"vdl-baidushare-{tid}", daemon=True).start()
    return {"task_id": tid, "name": name}
