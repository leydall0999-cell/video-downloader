...
def node_info() -> dict:
    ...
    """
    info = {'region': app.NODE_REGION, 'peer': app.PEER_ENDPOINT, 'china_domains': list(app.CHINA_DOMAINS), 'commentary_enabled': app.COMMENTARY_ENABLED, 'ads_enabled': app.ADS_ENABLED, 'convert': {'subscription_required': app.CONVERT_SUB_ENABLED, 'free_daily': app.CONVERT_FREE_DAILY, 'max_upload_bytes': app.UPLOAD_MAX_BYTES, 'targets': list(app.CONVERT_EXT.keys())}, 'download': {'subscription_required': app.DOWNLOAD_SUB_ENABLED, 'free_daily': app.DOWNLOAD_FREE_DAILY}, 'cloud': {'subscription_required': app.CLOUD_SUB_ENABLED, 'free_daily': app.CLOUD_FREE_DAILY, 'providers': ['webdav'] + (['baidu'] if app.BAIDU_ENABLED else []), 'baidu_available': app.BAIDU_ENABLED, 'baidu_auth_url': app.baidu_auth_url(app.BAIDU_REDIRECT_URI, app.BAIDU_APP_KEY, app_id=app.BAIDU_APP_ID) if app.BAIDU_ENABLED else ''}, 'library': {'enabled': app.plat.is_desktop() or bool(app.os.environ.get('VDL_LIBRARY_ENABLED'))}, 'subscriptions': {'enabled': app.SUB_ENABLED, 'probe_limit': app.SUBSCRIBE_PROBE_LIMIT, 'check_interval': app.SUB_CHECK_INTERVAL}, 'retention': {'enabled': app.RETENTION_ENABLED, 'trash_available': app.retention_mod.trash_available() if app.RETENTION_ENABLED else False}, 'archive': {'enabled': app.ARCHIVE_ENABLED, 'baidu_available': app.BAIDU_ENABLED, 'configured': app.archive_store.has_creds(app.archive_store.get().provider) if app.ARCHIVE_ENABLED else False}, 'crypto': {'enabled': app.CRYPTO_ENABLED, 'has_pass': bool(app._vault_load()) if app.CRYPTO_ENABLED else False, 'locked': app.VAULT_KEY is None}, 'torrent': {'enabled': app.TORRENT_ENABLED, 'available': app.torrent_mod.available()}, 'ai_dewatermark': {'enabled': app.AI_DEWATERMARK_ENABLED, 'gpu': app.AI_GPU_AVAILABLE}, 'authRequired': app.AUTH_REQUIRED, 'profile': ('web' if app.os.environ.get('VDL_INSTANCE') == 'cloud' else 'app')}
