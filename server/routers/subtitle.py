...
# 2核2G 服务器约束：ASR 加载+推理峰值 ~500MB-1GB，同时只允许一个任务，后续任务在信号量上排队
_SUBTITLE_SLOTS = threading.Semaphore(1)
...
def _run_subtitle(job_id: str, src: str, model_size: str, language: str, to_library: bool,
    ...
    txt_path = None
    wav_path = None
    _SUBTITLE_SLOTS.acquire()   # 单并发闸：排队等前一个 ASR 任务释放
    try:
...
# ------------------------------------------------------------------ 网页版上传路径（2026-09-11）
# 网页版没有本地路径概念：前端分片上传（/api/upload-chunk，32MB/片）后调本端点
# 合并分片并提交字幕任务。复用 convert.py 的分片落地机制（_upload_parts/UPLOAD_TMP）。

@router.post("/api/subtitle/finish")
def subtitle_upload_finish(
    upload_id: str = app.Form(...),
    total: int = app.Form(...),
    filename: str = app.Form("upload.mp4"),
    model_size: str = app.Form("small"),
    language: str = app.Form(""),
    fast: bool = app.Form(False),
    ...
    model_size = model_size if model_size in ALLOWED_MODELS else _DEFAULT_MODEL
    # 会员权益与本地版一致：下载/AI 会员满核，免费 4 线程
    try:
        is_member = bool(app.current_member_store(request).status()["download_member"]["active"])
    except Exception:
        is_member = False
    ...
    app.executor.submit(_run_subtitle, job_id, str(save_path), model_size,
                        (language or "").strip(), False, bool(fast), cpu_threads)
    record_event("subtitle", {"model": model_size, "member": is_member, "src": "upload"})
    return {"job_id": job_id, "status": "running", "model": model_size,
            "cpu_threads": cpu_threads, "member": is_member}
